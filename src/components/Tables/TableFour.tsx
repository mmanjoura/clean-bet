          ...
          {brandData.map((brand, index) => (
            <div
              className={`flex flex-col ${
                index === brandData.length - 1 ? "" : "border-b border-stroke dark:border-strokedark"
              }`}
              key={index}
            >
              <div className="grid grid-cols-3 sm:grid-cols-4 items-center">
                ...
                {/* Accordion Caret */}
                <div className="flex items-center justify-center p-2.5 xl:p-5 cursor-pointer" onClick={() => handleToggle(index)}>
                  <svg className={`w-5 h-5 transform transition-transform ${expandedRow === index ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              ...
              {expandedRow === index && (
                <div className="p-4 bg-gray-100 dark:bg-gray-800">
                  <p className="text-black dark:text-white">
                    Additional data for {brand.name} goes here.
                  </p>
